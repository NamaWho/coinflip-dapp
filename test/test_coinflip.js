...
contract("CoinFlip", async function(accounts){
    ...
    it("constructor(): 2 - should match its balance with on-blockchain balance", async () => {
        let balance = await instance.balance();
        balance = parseFloat(balance);
        
        let onChainBalance = await web3.eth.getBalance(instance.address);

        assert(balance == onChainBalance, "Balances not matching");
    })
    ...
    it("bet(): 2 - should limit bet amount to half of the total contract funds", async () => {
